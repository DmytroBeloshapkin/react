...
for (let i = 0; i < 10; i++) {
    document.write(`<div>довільний текст і індекс ${i}</div>`)
}
...
let j = 0
while (j < 20) {
    document.write(`<h1>довільним текстом всередині ${j + 1}</h1>`)
    j++
}
...
for (const product of products) {
    ...
    document.write(`<h3 class="product-title"> ${product.title} <br> ${product.price}</h3>`)
